...
timeoutSec = dict(
		   #  arpAware=60*2,
  arpAware=8,   # Quiet ARP-responding entries are pinged after this
  arpSilent=60*20, # This is for uiet entries not known to answer ARP
  arpReply=4,      # Time to wait for an ARP reply before retrial
  timerInterval=5, # Seconds between timer routine activations
  entryMove=60     # Minimum expected time to move a physical entry
...
class host_tracker (EventMixin):
  ...
  def updateIPInfo (self, pckt_srcip, macEntry, hasARP):
    ...
    if pckt_srcip in macEntry.ipAddrs:
      ...
      if kj_global.hostsDown['mac'] == str(macEntry):
	 kj_global.hostsDown['mac']="00:00:00:00:00:00" #for removing hosts that are working
      	 kj_global.hostsDown['ip']="0.0.0.0"
      log.info("filtering applid %s",kj_global.hostsDown['ip'])
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if macEntry is None:
      ...
      #log.info("**Dpid is*** %s", dpid) #added by jose
  ...
  def _check_timeouts (self):
    ...
    for macEntry in self.entryByMAC.values():
      ...
      for ip_addr, ipEntry in macEntry.ipAddrs.items():
	    ...
	    kj_global.hostsDown['ip']=str(ip_addr)
	    kj_global.hostsDown['mac']=str(macEntry) #added by jose
	    log.info("hostsDown inside host tracker %s,%s",kj_global.hostsDown['mac'],kj_global.hostsDown['ip'])
      ...
      if macEntry.expired() and not entryPinged:
        # log.info("Entry %s expired", str(macEntry))#commented by jose
